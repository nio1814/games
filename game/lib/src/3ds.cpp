...
void CLoad3DS::ReadUVCoordinates(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	pObject->pTexVerts = new Vector2D [pObject->numTexVertex];
...
{ 
	...
	memset(pObject->pVerts, 0, sizeof(Vector3D) * pObject->numOfVerts);
...
void CLoad3DS::ReadObjectMaterial(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	for(int i = 0; i < pModel->numOfMaterials; i++)
	{
		if(strcmp(strMaterial, pModel->pMaterials[i].strName) == 0)
		{
			...
			char * tempstr = pModel->pMaterials[i].strFile;
...
void CLoad3DS::ComputeNormals(t3DModel *pModel)
{
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		for (int i = 0; i < pObject->numOfVerts; i++)			
		{
			...
			pObject->pNormals[i] = pObject->pNormals[i].unit();
...
void getVertices(t3DObject* obj3ds, int idx, Vector3D vertices[])
{
	tFace face = obj3ds->pFaces[idx];

	for(int i=0; i<3; i++)
	{
		vertices[i] = obj3ds->pVerts[face.vertIndex[i]];
	}

	return;
}

Vector3D getNormal(t3DObject* obj3ds, int idx)
{
	Vector3D normal;
	//tFace face = obj3ds->pFaces[idx];

	normal = obj3ds->pNormals[obj3ds->pFaces[idx].vertIndex[0]];
